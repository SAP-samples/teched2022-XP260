...
2. In the "Add Resource" dialog you can now specify, which URL can be called from the cloud. Enter **/production/accounting** as the URL path and set the "AccessPolicy" to "Path Only", to prevent calls to sub-paths. Then click on the Save button.
